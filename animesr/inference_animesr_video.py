...
@torch.no_grad()
def inference_video(args, video_save_path, device=None, total_workers=1, worker_idx=0, model_id='kadirnar/AnimeSR_Paper_Model'):
    # prepare model
    model = get_inference_model(args, device, model_id=model_id)
...
def run(args, model_id='kadirnar/AnimeSR_Paper_Model'):
    ...
    if num_process == 1:
        ...
        inference_video(args, video_save_path, device=device, model_id=model_id)
        return
...
def main(source, model_id='kadirnar/AnimeSR_Paper_Model'):
